...
function AllSongsGallery({ songs, isDarkMode }) {
  ...
  const searchedList = songs.filter((song) =>
    song.title.toLowerCase().match(searchQuery.toLowerCase())
  );
  ...
  const filteredList =
    filterQuery === 'All'
      ? searchedList
      : searchedList.filter((song) => song.genre.includes(filterQuery));
  ...
  return (
    <div class={isDarkMode ? 'App' : 'App.light'}>
      <Search
        ...
        genres={[
          genres[0],
          ...genres.slice(1).sort((a, b) => a.localeCompare(b)),
        ]}
      />
      <SongList songs={filteredList} />
    </div>
  );
}
